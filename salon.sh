...
AVILABLE_SERVICES=$($PSQL "SELECT service_id,name from services order by service_id")
if [[ -z $AVILABLE_SERVICES ]]
then 
echo "Sorry we don't have any services available right now"
else
echo "$AVILABLE_SERVICES" | while read SERVICE_ID BAR NAME
do 
